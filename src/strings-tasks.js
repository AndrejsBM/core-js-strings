...
function removeFirstOccurrences(str, value) {
  ...
  return startIdx !== -1
    ? str.slice(0, startIdx) + str.slice(startIdx + value.length)
    : str;
}
...
function removeLastOccurrences(str, value) {
  const endIdx = str.lastIndexOf(value);

  return endIdx !== -1
    ? str.slice(0, endIdx) + str.slice(endIdx + value.length)
    : str;
}
...
function reverseWords(str) {
  ...
  for (let i = 0; i < string.length; i += 1) {
    ...
    for (let j = string[i].length - 1; j >= 0; j -= 1) {
      result += string[i][j];
    }
  }
